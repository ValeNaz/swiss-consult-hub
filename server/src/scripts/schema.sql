-- =====================================================
-- SCHEMA DATABASE: swiss_consult_hub
...
-- Procedura per aggiornare il conteggio richieste di un cliente
DELIMITER //
CREATE PROCEDURE IF NOT EXISTS update_client_request_count(IN p_client_id VARCHAR(36))
BEGIN
  UPDATE clients
  SET total_requests = (
    SELECT COUNT(*) FROM requests WHERE client_id = p_client_id
  )
...
-- Trigger per aggiornare il conteggio richieste dopo INSERT
DELIMITER //
...
BEGIN
  IF NEW.client_id IS NOT NULL THEN
    CALL update_client_request_count(NEW.client_id);
  END IF;
END //
DELIMITER ;

-- Trigger per aggiornare il conteggio richieste dopo UPDATE
DELIMITER //
...
BEGIN
  IF OLD.client_id IS NOT NULL THEN
    CALL update_client_request_count(OLD.client_id);
  END IF;
...
-- Trigger per aggiornare il conteggio richieste dopo DELETE
DELIMITER //
CREATE TRIGGER IF NOT EXISTS after_request_delete
